...
import type { BasicTarget, Options } from '../index';
  ...
  beforeEach(() => {
    ...
    jest.spyOn(document, 'addEventListener').mockImplementation((eventName, callback) => {
      if (eventName in events) {
        events[eventName as 'fullscreenchange' | 'fullscreenerror'].add(callback);
      }
    });
    jest.spyOn(document, 'removeEventListener').mockImplementation((eventName, callback) => {
      if (eventName in events) {
        events[eventName as 'fullscreenchange' | 'fullscreenerror'].delete(callback);
      }
    });
  });
